...
if (B_CACHE) 
{
    include_once PATH_SITE.DIR_SEP.'_cache'.DIR_SEP.ID_DOMAIN.DIR_SEP.$attributes['categoryid'].DIR_SEP.'a_category_article_latest.php';
    $i_count_article = count($a_category_article_latest);
    ...
    $a_article = array();
    for ($i=$i_article_from;$i<$i_article_to;$i++) if (isset($a_category_article_latest[$i])) $a_article[] = $a_category_article_latest[$i];
}
else
{
...
$a_navigator = getNavigator($i_count_article,$_CONFIG['article_page_limit'],$start,array('type'=>'latest','param'=> array('categoryid'=>$categoryid,'fulltitle'=>$a_category[$categoryid]['fulltitle'])));
$o_smarty->assign('a_navigator',$a_navigator);
$o_smarty->assign('start',$start);

//-- build html title for this page
$meta_title = _l('latest').' '.$a_category[$categoryid]['fulltitle'];
...
$meta_description = _l('latest').' '.$meta_description;
